...
def parse_dataset(N1, N2):
    ...
    for i, folder in enumerate(folders):
        ...
        for f in train_files:
            mesh = trimesh.load(f)
            points, face_indices = mesh.sample(N1, return_index=True)
        ...
        for f in test_files:
            mesh = trimesh.load(f)
            points, face_indices = mesh.sample(N2, return_index=True)
            
            test_points.append(points)
            test_labels.append(i)

    train_points = torch.tensor(np.array(train_points), dtype=torch.float32)
    test_points = torch.tensor(np.array(test_points), dtype=torch.float32)
