...
hdf5_data_dir = '/scratch/users/kashefi/KANmlp/hdf5_data'
...
class PointNetKAN(nn.Module):
    def __init__(self, input_channels, output_channels, scaling=SCALE):
        ...
        in_feats = int(1024 * scaling) + int(128 * scaling) + num_objects
        self.conv1 = nn.Conv1d(in_feats, 640, 1)
        self.bn1   = nn.BatchNorm1d(640)
        self.conv2 = nn.Conv1d(640, output_channels, 1)

    def forward(self, x, class_label):
        # encoder
        x = F.relu(self.bn4(self.jacobikan4(x)))
        local_4 = x

        x = F.relu(self.bn5(self.jacobikan5(x)))

        # global pooling
        global_feature = F.max_pool1d(x, kernel_size=x.size(-1))
        global_feature = global_feature.expand(-1, -1, num_points)

        # expand one-hot class
        class_label = class_label.view(-1, num_objects, 1).expand(-1, -1, num_points)

        # concat
        x = torch.cat([local_4, global_feature, class_label], dim=1)

        # decoder head: two shared-MLP layers
        x = F.relu(self.bn1(self.conv1(x)))
        x = self.conv2(x)  # [B, output_channels, num_points]
