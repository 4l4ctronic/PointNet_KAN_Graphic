...
###### Object: PointNetKAN for semantic segmentation (i.e., PointNet-KAN-MLP) ######
class PointNetKANMLP(nn.Module):
    def __init__(self, input_channels, output_channels, scaling=SCALE):
        super(PointNetKANMLP, self).__init__()
        ...
        in_feats = int(1024 * scaling) + int(128 * scaling)
    ...
    def forward(self, x):
        ...
        x = self.bn4(self.jacobikan4(x))

        local_4 = x

        x = self.bn5(self.jacobikan5(x))
...
def train(train_areas):
    ...
    model = PointNetKANMLP(9,NUM_CLASSES,SCALE).to(DEVICE)
